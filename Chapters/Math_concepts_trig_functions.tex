...
\begin{figure}
    ...
    \caption{The unit circle with some angles in degrees and radians.}
...
\textit{Exercise: Familiarize yourself some more with the unit circle here: \url{https://livephysics.com/tools/unit-circle-explorer/}}
...
Thus, the point on the unit circle corresponding to the angle $\alpha$ is described by the coordinates: $( \cos(\alpha), \sin(\alpha))$. 
...
As we saw in the last section, sine and cosine function are periodic with a period of $2\pi$. This can be seen in Fig. \ref{fig:sin_graph}.

\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{Bilder/sin_graph_period.jpeg}\includegraphics[width=0.45\linewidth]{Bilder/cos_graph_period.jpeg}
    \caption{Left: Graph of the sine function. The dashed black line shows the period. Right: Graph of the cosine function with its period.}
    \label{fig:sin_graph}
%Quelle : selbst mit Python
\end{figure}

We now list a few properties of the sine and cosine function.
...
For completeness' sake, Fig. \ref{fig:tan_graph} shows the graph of the tangent function. Since it is not particularly useful for describing waves, we only mention it here in passing. 

\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{Bilder/tan_graph_period.jpeg}
    \caption{Graph of the tangent function. The black dashed line shows its period.}
    \label{fig:tan_graph}
%Quelle : selbst mit Python
\end{figure}

\paragraph{Tangent Function}

\begin{itemize}
    \item \textbf{Domain}: $(-\infty, \infty) \backslash \{n\cdot \pi + \pi/2 | n \in \mathbb{Z}\}$  
    Any real number can be used as input.

    \item \textbf{Range}: $(-\infty, \infty)$ 
    The function can take all values.

    \item \textbf{Periodicity}:  
    The cosine function also repeats every $\pi$:
    \begin{equation}
        \tan(x + \pi) = \tan(x)
    \end{equation}
    \item \textbf{Odd function}:  
    The tangent function is symmetric with respect to the origin. This means:
    \begin{equation}
        \tan(-x) = -\tan(x)
    \end{equation}
\end{itemize}
...
This means that the cosine function is just the sine function shifted to the left by $\frac{\pi}{2}$ (a quarter of a full period), and vice versa. This is illustrated in Fig. \ref{fig:sin_cos_graph}.
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{Bilder/sin_cos_graph.jpeg}
    \caption{Graph of the sine function (blue) and the cosine function (red) in the same diagram. Note how the blue blue line is just the red line shifted to the right.}
    \label{fig:sin_cos_graph}
%Quelle : selbst mit Python
\end{figure}
...
\begin{equation}\label{eq:shifted_sine}
    f(x) = a \cdot \sin(bx + c) + d \quad \text{or} \quad f(x) = a \cdot \cos(bx + c ) + d
\end{equation}

Since cosine is just sine shifted, we can just pick one whenever we want to describe a ``sine-shaped'' function.  
...
The impact of changing these parameters are illustrated in Fig. \ref{fig:shifted_sines}.

\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{Bilder/shifted_sine_0.jpeg}
    \includegraphics[width=0.45\linewidth]{Bilder/shifted_sine_1.jpeg}
    \includegraphics[width=0.45\linewidth]{Bilder/shifted_sine_2.jpeg}
    \includegraphics[width=0.45\linewidth]{Bilder/shifted_sine_3.jpeg}
    \caption{Transformed sine functions for different parameters. The blue graph is the untransformed sine function.}
    \label{fig:shifted_sines}
%Quelle : selbst mit Python
\end{figure}
...
As we'll see in the next chapter, we can model many different types of waves and oscillations by changing the parameters $a$, $b$, $c$, and $d$. This is especially important in physics, for example when describing sound waves or vibrations.
